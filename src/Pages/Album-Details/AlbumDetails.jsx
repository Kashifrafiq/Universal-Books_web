...
const AlbumDetails = () => {
        ...
        <div className="container py-5">
          {albumDetail && (
            <>
                ...
                <div className="col-xl-8 col-lg-7 col-md-12 col-sm-12 px-0 album-card">
                  <div>
                    <div className="featured-subdata">
                      ...
                      <p className="description fs-6">{albumDetail.album_description}</p>
                      <div className="d-flex align-items-center album-price-btn">
                        {!albumDetail.purchased && (
                         ...
                         <table>
                            <tr>
                               <th>{albumDetail.album_category === 'Audio' ? 'Tracks' : 'Videos'}</th>
                               <th>Published Date</th>
                               <th>Released Date</th>
                              <th>Language</th>
                              <th>Type</th>
                              {albumDetail.album_ratings && <th>Album Ratings</th>}
                             </tr>
                             <tr>
                              <td>{albumDetail.tracks}</td>
                               <td>{new Date(albumDetail.published_date).toLocaleDateString()}</td>
                               <td>{new Date(albumDetail.released_date).toLocaleDateString()}</td>
                               <td>{albumDetail.languages}</td>
                               <td>{albumDetail.album_category}</td>
                               {albumDetail.album_ratings && <td>{albumDetail.album_ratings}+</td>}
                             </tr>
                           </table>
                         </div>
                      </div>
                            ...
                            <div >
                            <p className="price mb-0 fs-3">${albumDetail.album_price.toFixed(2)}</p>
                            <button
                              onClick={checkPopup}
                        ...
                        {albumDetail.purchased && (
                          <div className="d-flex align-items-center purchased">
                            <img src="assets/images/purchased-tick.png" alt="tick" />
              ...
              <div className="row">
                <div className="d-block px-0 featured-head border-bottom">
                  <p className="fs-3">{albumDetail.album_category === 'Audio' ? 'Tracks' : 'Videos'}</p>
                </div>
                <div className="album-tracks px-0">
                  <div className="table-responsive">                     <table className="table-tracks">
                      <tr className="fs-8">
                        <th>No.</th>
                        <th>Title</th>
                        <th>Artist</th>
                      ...
                      {albumDetail.contents.length > 0 ? (
                        ...
                        <div className="mt-3 text-medium-gray">
                          No {albumDetail.album_category === 'Audio' ? 'Tracks' : 'Videos'} Available
                        </div>
                      )}
                    </table>
                  </div>
  ...
  >
    ...
    >
      ...
      <h2 style={{ marginBottom: '20px', color: '#333' }}>Subscription Needed</h2>
      <p style={{ marginBottom: '30px', color: '#666' }}>
      The purchase of this album requires an active subscription. Please subscribe to a Monthly or Yearly Plan!
      </p>

        <button style={{all:"unset", marginLeft:"10px" , marginTop:"20px" , padding:"10px" , backgroundColor:"#44c8f5" , width:"130px" , textAlign:"center" , borderRadius:"30px" , cursor:"pointer"}} onClick={handle} >Browser Plan</button>
    </div>
  </div>
)}
